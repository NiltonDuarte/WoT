...
class Torre:
    ...
    def __init__(self, iniciarTorreFunc = False, pontos=0, listaParametros=[]):
	#Forca de disparo da Torre
        ...
        self.projeteis = [] #projeteis.append(Projetil())
        self.tropas = [] #tropas.append(Tropa())
        #Parte grafica------------------
        
        self.torreModel = None
        
        #----------------------------------
        
        if (len(listaParametros) > 0 and pontos and iniciarTorreFunc):
            self.pontosIniciais = pontos
            self.definirParametros(listaParametros)
            self.iniciarTorre()
